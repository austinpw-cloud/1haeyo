...
| Sprint | 기간 | 산출물 | 검증 |
|---|---|---|---|
| 0 | 3일 | Expo 프로젝트 + Supabase 연결 | 실기기 Hello World |
| 1 | 4일 | 카카오 로그인 + 프로필 | 로그인 성공, 프로필 생성 |
| 2 | 5일 | 일감 CRUD (구인자) | 일감 등록/수정/삭제 |
| 3 | 4일 | 워커 일감 조회 + 지원 | 주변 일감 보고 지원 |
| 4 | 5일 | Mode 1 (즉시 호출) | 5초 내 매칭 확정 |
| 5 | 5일 | Mode 2 (예약 구인) | 구인자 판정, 동시지원 취소 |
| 6 | 3일 | 지도 + GPS + 체크인 | 지도 표시, 백그라운드 추적 |
| 7 | 3일 | 평가 + 뱃지 | 리뷰 작성, 뱃지 부여 |
| 8 | 4일 | 정산 (PortOne) | 에스크로, 자동 송금 |
| 9 | 3일 | 페널티 시스템 | 포인트 누적, 제재 |
| 10 | 3일 | 시니어 UX 다듬기 | 실제 시니어 유저 테스트 |
| 11 | 3일 | Play Store 제출 | 심사 통과 |

**총 6주**. 각 Sprint 끝에 실제 작동 확인 후 다음 진행.
