...
DATABASE: Postgres · 6 core domain tables (profiles, jobs, applications, matches, reviews, contracts) — static signal scanners report 12 `create table` occurrences because `_all.sql` redeclares each table with `if not exists` for one-shot bootstrap on top of the per-feature 001–008 migrations · 36 RLS policies, gap_estimate=0 across all tables · 3 functions · 5 triggers · 6 tables on Realtime · 10 SQL files (001–009 + `_all.sql`); 009_fk_indexes adds matches.application_id, reviews.reviewer_id, reviews.job_id (commit.show static cross-file scan still reports gap=4 because it does not follow indexes added in later migrations — verified live in Supabase).
INFRA: EAS Build (`eas.json` with development / preview / production Android APK profiles, `NPM_CONFIG_LEGACY_PEER_DEPS=true`) · GitHub Actions CI: daily Supabase keepalive (`.github/workflows/supabase-keepalive.yml`) + test runner on push/PR to main (`.github/workflows/test.yml`) running ts-jest against pure-logic tests · no public deploy yet.
...
| Tests / CI | 75 | 25 | AI added ts-jest setup + 18 tests covering pricing/format pure-logic + GitHub Actions test workflow on push/PR; human picked the test targets and the audit-driven prioritization. Component/integration tests still 0 — honest residual gap. |
...
- Schema object counts: `grep` over `supabase/schema/*.sql` — 6 core domain tables (12 `create table` occurrences across files due to `_all.sql` redeclaration), 36 distinct RLS policies (gap_estimate=0), 3 functions, 5 triggers, 10 SQL files. 009_fk_indexes adds matches.application_id, reviews.reviewer_id, reviews.job_id — applied to live Supabase database 2026-05-01.
- EAS profiles: read from `eas.json` (development / preview / production, Android APK, `NPM_CONFIG_LEGACY_PEER_DEPS=true`).
- CI: `.github/workflows/supabase-keepalive.yml` runs daily at 03:00 UTC against the REST API to prevent free-tier auto-pause; `.github/workflows/test.yml` runs `npm test` (ts-jest) on every push/PR to main against 18 unit tests in `shared/utils/*.test.ts`.
